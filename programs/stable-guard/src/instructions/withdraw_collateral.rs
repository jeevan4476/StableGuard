...
impl<'info> WithdrawalCollateral<'info> {
    pub fn withdraw(
        ...
        require!(
            lp_amt_to_burn > 0,
            StableGuardError::WithdrawalConditionsNotMet
        );
        require!(
        ...
        let usdc_to_withdraw_u128 =
            (lp_amt_to_burn as u128 * total_usdc_pool as u128) / total_lp_supply as u128;

        let usdc_to_withdraw = usdc_to_withdraw_u128 as u64;
        require!(usdc_to_withdraw > 0, StableGuardError::NolpTokensToBurn);
        ...
        burn_checked(cpi_ctx, lp_amt_to_burn, self.lp_mint.decimals)?;
